# Day 184: CI/CD：GitHub Actions → 构建 → ArgoCD 部署

> 📅 日期：2026-05-02  
> 📖 学习主题：CI/CD：GitHub Actions → 构建 → ArgoCD 部署  
> ⏰ 计划学习时间：2-3 小时
...
完成 Day 184 的学习后，你应该掌握：
- 理解 CI/CD：GitHub Actions → 构建 → ArgoCD 部署 的核心概念和原理
- 能够独立完成相关命令的操作练习
- 在实际工作中正确应用这些知识
- 为 SRE 进阶打下坚实基础
...
### 1. CI/CD：GitHub Actions → 构建 → ArgoCD 部署
...
### 练习 1：CI/CD：GitHub Actions → 构建 → ArgoCD 部署 实战

**场景**：将 CI/CD：GitHub Actions → 构建 → ArgoCD 部署 应用到生产环境中。
...
```bash
# 为 CI/CD：GitHub Actions → 构建 → ArgoCD 部署 配置监控指标
# 设置告警阈值
# 编写健康检查脚本
```
...
*由 SRE 学习计划自动生成 | 2026-05-02 15:07:43*  
